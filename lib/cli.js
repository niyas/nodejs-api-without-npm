 ...
 */
 ...
 cli.responders.help = function() {
    var commands = {
        ...
        'stats': 'Get status of underlying operating system and resource utilization',
        'list users': 'Shows a list of all registerd users in the system',
        'more user info --{userId}': 'Show details of a specific user',
        'list checks --up --down': 'Show the list of all active checks in the system, including their state. The "--up" and the "--down" are both optional',
    ...
    // Show each command, followed by its explanation in white and yello respectively
    ...
    // End with another horizondal line
    cli.horizondalLine();
 };

 // Create vertical space
 cli.verticalSpace = function(lines) {
     lines = typeof(lines) == 'nimber' && lines > 0 ? lines : 1;
     for(var i=0; i < lines; i++) {
         console.log('');
     }
 };

 // Create a horizondal line across the screen
 cli.horizondalLine = function() {
 ...
 // Centere the heading on the screen
            ...
            checkIds.forEach(checkId => {
                _data.read('checks', checkId, function(err, checkData) {
                    if(!err && checkData) {
                        ...
                        // If the user has specified the state, hasen't specified any state, include the current state
                        if(lowerString.indexOf('--' + state) > -1 || (lowerString.indexOf('--up') == -1 && lowerString.indexOf('--down'))) {
